"""Build an original icon and a minimal Windows COFF .rsrc (no external tools)."""
from pathlib import Path
from PIL import Image, ImageDraw
import io, struct
ROOT=Path(__file__).resolve().parents[1]
# Original geometric paper/check icon. No font files or third-party images.
im=Image.new('RGBA',(256,256),(0,0,0,0));d=ImageDraw.Draw(im)
d.rounded_rectangle((4,4,252,252),radius=58,fill='#13283D')
d.rounded_rectangle((65,38,191,209),radius=13,fill='#F7FAFC')
d.polygon([(153,38),(191,77),(153,77)],fill='#B7D4D3')
for y,w in [(94,66),(114,78),(134,44)]:d.rounded_rectangle((85,y,85+w,y+7),radius=3,fill='#8FAAB6')
d.ellipse((129,144,229,244),fill='#14A88B')
d.line([(152,194),(174,214),(208,176)],fill='white',width=13,joint='curve')
im.save(ROOT/'assets/app.ico',format='ICO',sizes=[(16,16),(32,32),(48,48),(64,64),(128,128),(256,256)])
...
<assemblyIdentity version="1.1.0.0" processorArchitecture="amd64" name="ApplyKit.Desktop" type="win32"/>
...
def block(key,value=b'',children=(),typ=0,units=None):
 b=pad(struct.pack('<HHH',0,len(value) if units is None else units,typ)+u16(key))
 b+=value
 if children:b=pad(b)+b''.join(pad(x) for x in children)
 return struct.pack('<H',len(b))+b[2:]
strings={'CompanyName':'ApplyKit','FileDescription':'投递材料助手 · 离线转换与压缩','FileVersion':'1.1.0.0','InternalName':'ApplyKit','OriginalFilename':'ApplyKit.exe','ProductName':'ApplyKit 投递材料助手','ProductVersion':'1.1.0','LegalCopyright':'ApplyKit 2026 - see included source license'}
strtable=block('040904B0',children=[block(k,u16(v),typ=1,units=len(u16(v))//2) for k,v in strings.items()],typ=1)
stringinfo=block('StringFileInfo',children=[strtable],typ=1)
varinfo=block('VarFileInfo',children=[block('Translation',struct.pack('<HH',1033,1200))],typ=1)
fixed=struct.pack('<13I',0xFEEF04BD,0x10000,0x10001,0,0x10001,0,0x3f,0,0x40004,1,0,0,0)
resources[(16,1,1033)]=block('VS_VERSION_INFO',fixed,[stringinfo,varinfo])
# Build the 3-level resource directory; offsets below are relative to .rsrc.
...
def directory(tree,depth=0):
 ...
 for i,ident in enumerate(ids):
  ...
  if depth<2:child=directory(subset,depth+1)|0x80000000
   ...
   align();child=reserve(16);data_entries.append((child,next(iter(subset.values()))))
...
header=struct.pack('<HHIIIHH',0x8664,1,0,sym_off,1,0,0)
section=struct.pack('<8sIIIIIIHHI',b'.rsrc\0\0\0',0,0,len(raw),raw_off,reloc_off,0,len(relocs),0,0x40300040)
...
print('Windows resources:',len(raw),'bytes;',len(resources),'resources')
